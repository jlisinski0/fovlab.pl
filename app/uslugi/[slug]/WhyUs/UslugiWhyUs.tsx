import UslugiWhyUsBlock from './UslugiWhyUsBlock'
import { UslugiWhyUsItem } from '@/types'

export default function UslugiWhyUs(item: UslugiWhyUsItem) {
	const { block } = item
	...
	return (
		<section className='h-auto bg-midnightTwo/99'>
			<div className='flex flex-col  h-full w-full max-w-7xl mx-auto px-10 py-20 '>
				...
				<div className='flex flex-wrap gap-5'>{block && block.map(subBlock => <UslugiWhyUsBlock key={subBlock.id} {...subBlock} />)}</div>
			</div>
		</section>
	)
}
